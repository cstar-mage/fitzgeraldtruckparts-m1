...
class AW_Helpdesk3_Model_Ticket_History_Event_Priority extends AW_Helpdesk3_Model_Ticket_History_Event_Abstract
{
    const TYPE = 6;
    ...
    public function isEventHappened($ticket)
    {
        if (!$ticket->isObjectNew() && $ticket->getOrigData('priority') != $ticket->getPriority()) {
            $this->setEventData(
                array(
                    'from' => $ticket->getOrigData('priority'),
                    'to'   => $ticket->getPriority(),
                )
            );
            return true;
        }
