...
class AW_Helpdesk3_Model_Gateway extends Mage_Core_Model_Abstract
{
        ...
        if (count($mailboxUIDs) > 0) {
            ...
            foreach ($mailboxUIDs as $messageUid) {
                try {
                    ...
                    $from = str_replace('>', '"',str_replace('<', ' "', $this->_getMessageFrom($message)));
                    Mage::log(
                        $e->getMessage() . ' in email from ' . $from,
                        null,
                        'awhdu3_gateway_errors.log'
                    );
                    AW_Lib_Helper_Log::log(
                        $e->getMessage() . ' in email from ' . $from,
                        AW_Lib_Helper_Log::SEVERITY_ERROR
                    );
                    continue;
                }
            }
    ...
    protected function _getMessageSubject($message)
    {
        ...
        if ($message->headerExists('subject')) {
            $subject = $this->_decodeMimeHeader($message->subject);
        }
    ...
    {
        $from = $this->_decodeMimeHeader($message->from);
        if (!$from) {
    ...
    protected function _decodeMimeHeader($value)
    {
