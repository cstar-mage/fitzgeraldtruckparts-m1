...
class AW_Helpdesk3_Adminhtml_Awhdu3_TicketController extends Mage_Adminhtml_Controller_Action
{
...
//            $collection->addAdminUserFilter(Mage::getSingleton('admin/session')->getUser());
                ...
                if ($ticket->isReadOnly()) {
                    Mage::getSingleton('adminhtml/session')->addNotice(
                        $this->__('Can not assign ticket <a href="%s" target="_blank">[%s]</a> it\'s locked',
                            Mage::getSingleton('adminhtml/url')->getUrl('adminhtml/awhdu3_ticket/edit',
            ...
            if (count($agentDepartmentIds) == 0) {
                throw new Mage_Core_Exception(
                    $this->__('Can not assign tickets to selected agent, because this agent'
                        ...
                        Mage::getSingleton('adminhtml/url')->getUrl('adminhtml/awhdu3_department')
                    )
                );
            }
    ...
    {
        $orderIncrementId = $this->getRequest()->getParam('order_increment_id', null);
        $order = Mage::getModel('sales/order')->loadByIncrementId($orderIncrementId);
        $orderId = $order->getEntityId();
        if (is_null($orderId)) {
            Mage::getSingleton('adminhtml/session')->addError($this->__("Unable to find the order"));
            $this->_redirect('*/*/list');
            return;
        }
    ...
    public function ajaxChangeCustomerInfoAction()
    {
        ...
        $text = trim(Mage::helper('aw_hdu3')->escapeHtml($content));
        if (!trim($content)) {
            $text = $this->__('No data');
        }
        $result['data'] = array(
            'text' => $text,
            'originalText' => Zend_Json::encode($text)
        );
        $this->_checkLocked();
        $this->getResponse()->setBody(
            Zend_Json::encode($result)
    ...
    public function ajaxGetAssigneePopupPropertiesAction()
    {
        $this->_initTicket();
        $ticket = Mage::registry('current_ticket');
        ...
        $url = Mage::helper('adminhtml')->getUrl(
            'adminhtml/awhdu3_ticket/ajaxChangeAssignee',
            array('id' => $ticket->getId())
        );
        $result = array(
            'titles' => $titles,
