...
class AW_Lib_Model_AdminhtmlPdf_Sales_Order_Pdf_Creditmemo extends Mage_Sales_Model_Order_Pdf_Creditmemo
{
    ...
    {
        $pdfCreditmemoNode = Mage::getConfig()->getNode("aw_lib/models/pdf/creditmemo");
        if ($pdfCreditmemoNode) {
            $configData = $pdfCreditmemoNode->asArray();
     ...
     */
    protected function insertOrder(&$page, $obj, $putOrderId = true, $creditmemo = NULL)
    {
        ...
        if($creditmemo){
            $page->drawText(
                Mage::helper('sales')->__('Refund Date: ').$this->getCreditMemoDateForPdfFormatted($creditmemo),
                35,
                ($top -= 15),
                'UTF-8'
            );
    ...
    public function getCreditMemoDateForPdfFormatted($creditMemo)
    {
        $creditMemoCreatedAt = $creditMemo->getCreatedAt();
        $createdAtObj = DateTime::createFromFormat('Y-m-d H:i:s', $creditMemoCreatedAt);
        $createdAtFormatted = $createdAtObj->format('M d, Y');
    ...
    protected function insertTotals($page, $source){
        ...
        $isAdminPdfCreditmemo = $this->_isAdminPdfCreditmemo();
        if($isAdminPdfCreditmemo && !empty($totals)){
            $coreHelper = Mage::helper('core');
            foreach($totals as $total){
                $sourceField = $total->getSourceField();
                if($sourceField ){
                    if($sourceField == 'subtotal'){
                        $total->setTitle($coreHelper->__('Refund Subtotal'));
                    }
                    if($sourceField == 'grand_total'){
                        $total->setTitle($coreHelper->__('Refund Grand Total'));
                    }
                }
            }
        }
    ...
    protected function _isAdminPdfCreditmemo()
    {
        $request = Mage::app()->getRequest();
        $moduleController = $request->getControllerName();
        $moduleControllerAction = $request->getActionName();
        if($moduleController == 'sales_order_creditmemo' && $moduleControllerAction == 'print'){
            return true;
        }
        return false;
