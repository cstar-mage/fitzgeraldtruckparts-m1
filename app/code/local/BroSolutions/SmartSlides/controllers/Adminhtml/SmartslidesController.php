...
class BroSolutions_SmartSlides_Adminhtml_SmartslidesController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function editslidersgroupAction()
    {
        ...
        Mage::register('slides_group_model_id', $groupId);
        $this->loadLayout();
        ...
        if($sliderGroupData){
            try {
                if(isset($sliderGroupData['form_slides'])){
                    if(strpos($sliderGroupData['form_slides'], '&') !== false){
                        $slidesIds = explode('&', $sliderGroupData['form_slides']);
                        if(!empty($slidesIds)){
                            $sliderGroupData['slides_ids'] = serialize($slidesIds);
                        }
                    } else {
                        $slidesIds =  array($sliderGroupData['form_slides']);
                        $sliderGroupData['slides_ids'] = serialize($slidesIds);
                    }
                }
                ...
                /*if($sliderGroupData['auto_play'] == 'true'){
                    $slidersGroupsModel->setAutoPlay(1);
                } else {
                    $slidersGroupsModel->setAutoPlay(0);
                }
                if($sliderGroupData['pause_on_hower'] == 'true'){
                    $slidersGroupsModel->setPauseOnHower(1);
                } else {
                    $slidersGroupsModel->setPauseOnHower(0);
                }*/
    ...
    public function saveslideAction()
    {
        ...
        if($sliderData){
            try {
                ...
                Mage::getSingleton('core/session')->addSuccess('Slide has been saved.');
