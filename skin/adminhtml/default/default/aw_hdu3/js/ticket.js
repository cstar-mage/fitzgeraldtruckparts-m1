...
var awHDU3TicketGrid = {
    ...
    openAssigneePopup: function(url, id){
        if (awHDU3TicketGridAssigneePopupList.hasOwnProperty(id)) {
            ...
            new Ajax.Request(url, {
                onComplete: function (transport) {
                    if (transport.responseText.isJSON()) {
                        var response = transport.responseText.evalJSON();
                        awHDU3TicketGridAssigneePopupList[id] = new AWHDU3_TICKET_POPUP(
                            response.content,
                            function(){me.changeAssignee(response.url, id);},
                            response.titles
                        );
                        awHDU3TicketGridAssigneePopupList[id].show();
                    }
